...
namespace engine::time {
    ...
    struct VariableTimestepConfig {
        // Delta time limits
        Duration minDeltaTime{constants::MIN_DELTA_TIME}; ///< Minimum delta (prevents div by zero)
        Duration maxDeltaTime{constants::MAX_DELTA_TIME}; ///< Maximum delta (prevents spiral)

        // Smoothing configuration
        SmoothingMethod smoothing{SmoothingMethod::EXPONENTIAL}; ///< Delta smoothing algorithm
        std::size_t smoothingWindowSize{5}; ///< Samples for smoothing
        double smoothingFactor{0.9}; ///< EMA smoothing factor [0,1]
        bool enableSmoothing{true}; ///< Enable delta smoothing

        // Frame rate targets
        std::optional<double> targetFPS{}; ///< Optional target FPS
        Duration targetFrameTime{constants::TARGET_FRAME_TIME_60FPS}; ///< Target frame duration

        // Spike detection
        bool detectSpikes{true}; ///< Enable frame spike detection
        double spikeThreshold{2.0}; ///< Spike threshold multiplier (vs average)

        // Time scaling and behavior
        TimeScale timeScale{1.0}; ///< Time scale factor [0, MAX_TIME_SCALE]
        bool allowNegativeTime{false}; ///< Allow time reversal
        bool compensateFrameDrops{true}; ///< Adjust for dropped frames

        // Interpolation
        bool enableInterpolation{true}; ///< Enable sub-frame interpolation
        double interpolationAlpha{0.0}; ///< Current interpolation factor [0,1]

        // Platform options
        bool useHighPrecisionTimer{true}; ///< Use high-resolution platform timer (QPC on Windows)
    };
