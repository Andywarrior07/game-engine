...
namespace engine::camera {
    ...
    Vec2 Viewport::normalizedToViewport(const Vec2& normalized) const {
        return Vec2(
    ...
    Vec2 Viewport::viewportToNormalized(const Vec2& viewport) const {
        return Vec2(
    ...
    bool Viewport::contains(const Vec2& point) const {
        return contains(static_cast<int>(point.x), static_cast<int>(point.y));
    }

    Vec2 Viewport::clamp(const float x, const float y) const {
        return Vec2(
