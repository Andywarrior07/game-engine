...
CircularBufferAllocator::CircularBufferAllocator(const MemorySize capacity,
                                                 const std::size_t maxEntries,
                                                 const char *name)
    : memory_(nullptr), capacity_(capacity),
      maxEntries_(maxEntries > 0 ? maxEntries
                                 : capacity / 64) // Default to 64-byte entries
      ,
      writeHead_(0), readHead_(0), entryCount_(0), overwriteCount_(0),
      name_(name) {
...
CircularBufferAllocator::CircularBufferAllocator(
    CircularBufferAllocator &&other) noexcept
    : memory_(other.memory_), capacity_(other.capacity_),
      maxEntries_(other.maxEntries_), writeHead_(other.writeHead_.load()),
      readHead_(other.readHead_.load()), entryCount_(other.entryCount_.load()),
      overwriteCount_(other.overwriteCount_.load()), name_(other.name_),
...
void *CircularBufferAllocator::allocate(const MemorySize size,
  ...
  if (const MemorySize currentRead = readHead_.load(std::memory_order_acquire);
      currentWrite <= currentRead && currentWrite + totalSize > currentRead) {
  ...
  // Update write head
  writeHead_.store((currentWrite + totalSize) % capacity_,
                   std::memory_order_release);

  // Track entry
  const std::size_t entryIndex =
      entryCount_.fetch_add(1, std::memory_order_acq_rel) % maxEntries_;
...
void CircularBufferAllocator::deallocate(void *ptr) {
  ...
  if (ptr) {
    ...
    if (!warningShown) {
      std::cerr << "Warning: CircularBufferAllocator does not support "
...
MemorySize CircularBufferAllocator::getUsedMemory() const {
  ...
  if (const MemorySize read = readHead_.load(std::memory_order_acquire);
      write >= read) {
    return write - read;
  } else {
...
MemorySize CircularBufferAllocator::getAllocationSize(const void *ptr) const {
  ...
  const std::uint8_t *memStart =
      reinterpret_cast<const std::uint8_t *>(memory_);

  // Search backwards for header within reasonable distance
  for (MemorySize offset = sizeof(EntryHeader); offset <= 256;
       offset += alignof(EntryHeader)) {
    if (bytePtr - offset < memStart)
      break;

    const EntryHeader *header =
        reinterpret_cast<const EntryHeader *>(bytePtr - offset);
