...
PoolAllocator::PoolAllocator(const MemorySize blockSize,
                             ...
                             const MemorySize alignment, const char *name)
    : memory_(nullptr),
      blockSize_(
          alignSize(blockSize, alignment)) // Ensure block size is aligned
      ,
      blockCount_(blockCount), alignment_(alignment), freeList_(nullptr),
      usedBlocks_(0), name_(name) {
  // Validate parameters
  assert(isPowerOfTwo(alignment) && "Alignment must be power of 2");
  assert(blockSize >= sizeof(FreeNode) &&
...
PoolAllocator::~PoolAllocator() {
  ...
  if (usedBlocks_.load() > 0) {
    std::cerr << "Warning: PoolAllocator '" << name_ << "' destroyed with "
              << usedBlocks_.load() << " blocks still allocated!" << std::endl;
...
PoolAllocator::PoolAllocator(PoolAllocator &&other) noexcept
    : memory_(other.memory_), blockSize_(other.blockSize_),
      blockCount_(other.blockCount_), alignment_(other.alignment_),
      freeList_(other.freeList_.load()), usedBlocks_(other.usedBlocks_.load()),
      name_(other.name_) {
...
void *PoolAllocator::allocate(const MemorySize requestedSize,
                              const MemorySize requestedAlignment,
  ...
  while (head != nullptr) {
    if (FreeNode *next = head->next; freeList_.compare_exchange_weak(
...
void PoolAllocator::deallocate(void *ptr) {
  ...
  if (!owns(ptr)) {
    std::cerr
        << "Error: Attempted to deallocate pointer not owned by this pool!"
        << std::endl;
...
bool PoolAllocator::owns(const void *ptr) const {
  ...
  if (const std::uint8_t *memEnd = memStart + (blockSize_ * blockCount_);
      bytePtr < memStart || bytePtr >= memEnd) {
    return false;
  }
