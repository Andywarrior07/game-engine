 ...
 *          Uses a circular buffer strategy where new allocations overwrite the
 * oldest data when the buffer is full. Thread-safe with atomic operations for
 * read/write heads.
 *
...
public:
   ...
   */
  explicit CircularBufferAllocator(
      MemorySize capacity, std::size_t maxEntries = 0,
   ...
   */
  bool isFull() const {
    return static_cast<float>(getUsedMemory()) >=
           (static_cast<float>(capacity_) * 0.9f); // 90% threshold
  }
