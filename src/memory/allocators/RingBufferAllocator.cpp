...
RingBufferAllocator::RingBufferAllocator(const MemorySize capacity,
                                         const char *name)
    : memory_(nullptr), capacity_(capacity), head_(0), tail_(0),
      fenceCounter_(0), name_(name) {
...
RingBufferAllocator::~RingBufferAllocator() {
  ...
  if (used > 0) {
    std::cerr << "Warning: RingBufferAllocator '" << name_
              << "' destroyed with " << used << " bytes still allocated!"
              << std::endl;
  }
#endif
...
RingBufferAllocator::RingBufferAllocator(RingBufferAllocator &&other) noexcept
    : memory_(other.memory_), capacity_(other.capacity_),
      head_(other.head_.load()), tail_(other.tail_.load()),
      fenceCounter_(other.fenceCounter_.load()), name_(other.name_) {
...
void *RingBufferAllocator::allocate(const MemorySize size,
...
void RingBufferAllocator::deallocate(void *ptr) {
  ...
  if (ptr) {
    ...
    if (!warningShown) {
      std::cerr << "Warning: RingBufferAllocator does not support individual "
                   "deallocation. "
...
MemorySize RingBufferAllocator::getAllocationSize(const void *ptr) const {
  ...
  for (MemorySize offset = sizeof(AllocationHeader); offset <= 256;
       offset += alignof(AllocationHeader)) {
    const AllocationHeader *header =
        reinterpret_cast<const AllocationHeader *>(bytePtr - offset);
