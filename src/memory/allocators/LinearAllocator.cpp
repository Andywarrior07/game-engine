...
LinearAllocator::LinearAllocator(const MemorySize capacity, const char *name)
    : memory_(nullptr), capacity_(capacity), current_(0), allocationCount_(0),
      name_(name) {
...
#ifdef _DEBUG
  if (current_.load() > 0) {
    std::cerr << "Warning: LinearAllocator '" << name_ << "' destroyed with "
...
LinearAllocator::LinearAllocator(LinearAllocator &&other) noexcept
    : memory_(other.memory_), capacity_(other.capacity_),
      ...
      allocationCount_(other.allocationCount_.load()), name_(other.name_) {
...
void *LinearAllocator::allocate(const MemorySize size,
...
void LinearAllocator::deallocate(void *ptr) {
  ...
  if (!warningShown) {
    std::cerr
        << "Warning: LinearAllocator does not support individual deallocation. "
