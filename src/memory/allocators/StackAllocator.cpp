...
StackAllocator::StackAllocator(MemorySize capacity, const char *name)
    : memory_(nullptr), capacity_(capacity), current_(0), highWaterMark_(0),
      name_(name) {
  memory_ = std::aligned_alloc(CACHE_LINE_SIZE, capacity);

  if (!memory_) {
    std::cerr << "[StackAllocator] Failed to allocate "
              << (capacity / (1024.0 * 1024.0)) << " MB for " << name_
              << std::endl;
    throw std::bad_alloc();
  }
...
#ifdef _DEBUG
  if (current_.load() > 0) {
    std::cerr << "Warning: StackAllocator '" << name_ << "' destroyed with "
              << current_.load() << " bytes still allocated!" << std::endl;
  }
#endif
...
StackAllocator::StackAllocator(StackAllocator &&other) noexcept
    : memory_(other.memory_), capacity_(other.capacity_),
      ...
      highWaterMark_(other.highWaterMark_.load()), name_(other.name_) {
...
void *StackAllocator::allocate(const MemorySize size,
  ...
  while (!current_.compare_exchange_weak(
      expected, newPos, std::memory_order_acq_rel, std::memory_order_acquire)) {
    ...
    const MemorySize newAdjustment =
        static_cast<std::uint8_t *>(alignedAddress) -
        static_cast<std::uint8_t *>(currentAddress);
  ...
  while (newPos > currentHigh &&
    ...
    // Keep trying
    /*
     * Esto es por si llega a haber un pequeño fallo en compare_exchange_weak
     * (que es el comportamiento normal) se reintenta por el bucle
     */
  }
...
void StackAllocator::deallocate(void *ptr) {
  ...
  if (header->sentinel != SENTINEL_VALUE) {
    std::cerr << "Error: Corrupted allocation header in StackAllocator!"
              << std::endl;
  ...
  MemorySize allocEnd =
      allocStart + header->adjustment + sizeof(AllocationHeader) + header->size;
  ...
  if (allocEnd == current_.load(std::memory_order_acquire)) {
    ...
    recordDeallocation(header->size + header->adjustment +
    ...
    std::cerr
        << "Warning: Attempted to free non-top allocation in StackAllocator. "
...
void StackAllocator::freeToMarker(const Marker marker) {
  ...
  if (marker > currentPos) {
    std::cerr << "Error: Invalid marker in StackAllocator::freeToMarker!"
              << std::endl;
    return;
  }
