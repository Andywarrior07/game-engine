...
TimerPoolAllocator::TimerPoolAllocator(
    const std::array<PoolConfig, static_cast<std::size_t>(TimerType::COUNT)>
        &configs,
    const char *name)
    : configs_(configs), name_(name), totalCapacity_(0), totalUsed_(0) {
  ...
  for (std::size_t i = 0; i < static_cast<std::size_t>(TimerType::COUNT); ++i) {
    ...
    try {
      pools_[i] = std::make_unique<PoolAllocator>(
          config.blockSize, config.initialCount, DEFAULT_ALIGNMENT,
          (std::string(name_) + "_" +
           getTimerTypeName(static_cast<TimerType>(i)))
              .c_str());
  ...
  std::cout << "[TimerPoolAllocator] Initialized with "
            << (static_cast<double>(totalCapacity_) / (1024.0 * 1024.0))
...
TimerPoolAllocator::~TimerPoolAllocator() {
  ...
  for (std::size_t i = 0; i < static_cast<std::size_t>(TimerType::COUNT); ++i) {
    if (pools_[i] && pools_[i]->getUsedMemory() > 0) {
      std::cerr << "Warning: TimerPoolAllocator pool "
                << getTimerTypeName(static_cast<TimerType>(i)) << " has "
                << pools_[i]->getUsedMemory() << " bytes still allocated!"
                << std::endl;
    }
  }
#endif
...
void *TimerPoolAllocator::allocate(const MemorySize size, MemorySize alignment,
...
void *TimerPoolAllocator::allocateTimer(const TimerType timerType,
                                        const AllocationFlags flags) {
  return allocateFromPool(timerType, flags);
}

std::vector<void *>
TimerPoolAllocator::batchAllocateTimers(const TimerType timerType,
...
#ifdef _DEBUG
  std::cerr << "Error: Attempted to deallocate pointer not owned by "
               "TimerPoolAllocator!"
            << std::endl;
...
float TimerPoolAllocator::getPoolUtilization(TimerType timerType) const {
  ...
  return static_cast<float>(pool->getUsedMemory()) /
...
bool TimerPoolAllocator::growPool(TimerType timerType) {
  ...
  std::size_t newCount =
      currentCount * static_cast<std::size_t>(config.growthFactor);
  ...
  try {
    ...
    auto newPool = std::make_unique<PoolAllocator>(
        config.blockSize, newCount, DEFAULT_ALIGNMENT,
        (std::string(name_) + "_" + getTimerTypeName(timerType) + "_grown")
            .c_str());

    // Update capacity tracking
    const MemorySize additionalCapacity =
        config.blockSize * (newCount - currentCount);
...
std::string TimerPoolAllocator::generateDetailedReport() const {
  ...
  report << "Total Used: "
         ...
         << std::endl;
  report << "Overall Utilization: " << (getUtilization() * 100.0f) << "%"
         << std::endl;
  report << std::endl;
  ...
  for (std::size_t i = 0; i < static_cast<std::size_t>(TimerType::COUNT); ++i) {
    ...
    report << "  Block Size: " << configs_[i].blockSize << " bytes"
           << std::endl;
    report << "  Total Blocks: " << pool->getTotalBlockCount() << std::endl;
    report << "  Used Blocks: "
           << (pool->getTotalBlockCount() - pool->getFreeBlockCount())
           << std::endl;
    report << "  Free Blocks: " << pool->getFreeBlockCount() << std::endl;
    report << "  Utilization: " << (getPoolUtilization(type) * 100.0f) << "%"
           << std::endl;
    report << "  Memory: "
           << (static_cast<double>(pool->getUsedMemory()) / 1024.0) << " / "
           << (static_cast<double>(pool->getCapacity()) / 1024.0) << " KB"
           << std::endl;
    report << std::endl;
  }
...
void *TimerPoolAllocator::allocateFromPool(TimerType timerType,
                                           const AllocationFlags flags) {
  if (timerType >= TimerType::COUNT) {
  ...
  const MemorySize blockSize =
      configs_[static_cast<std::size_t>(timerType)].blockSize;
...
TimerPoolAllocator::TimerType
TimerPoolAllocator::selectTimerTypeBySize(const MemorySize size) const {
  // Find the smallest pool that can accommodate the size
  auto bestType = TimerType::ONE_SHOT;
  MemorySize bestFit =
      configs_[static_cast<std::size_t>(TimerType::ONE_SHOT)].blockSize;

  for (std::size_t i = 0; i < static_cast<std::size_t>(TimerType::COUNT); ++i) {
    if (const MemorySize blockSize = configs_[i].blockSize;
        blockSize >= size && blockSize < bestFit) {
      bestFit = blockSize;
