...
void *IAllocator::reallocate(void *ptr, const MemorySize newSize,
...
MemorySize IAllocator::alignSize(const MemorySize size,
                                 const MemorySize alignment) {
...
void IAllocator::recordAllocation(const MemorySize size,
                                  const MemoryCategory category) const {
  ...
  stats_.categoryUsage[static_cast<std::size_t>(category)].fetch_add(
      size, std::memory_order_relaxed);

  // Update peak usage
  const MemorySize current =
      stats_.currentUsage.load(std::memory_order_relaxed);
...
void IAllocator::recordDeallocation(const MemorySize size,
                                    const MemoryCategory category) const {
  stats_.totalFreed.fetch_add(size, std::memory_order_relaxed);
  stats_.currentUsage.fetch_sub(size, std::memory_order_relaxed);
  stats_.categoryUsage[static_cast<std::size_t>(category)].fetch_sub(
      size, std::memory_order_relaxed);
}
} // namespace engine::memory
