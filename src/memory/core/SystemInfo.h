...
struct SystemInfo {
  ...
  std::size_t recommendedMaxMemory =
      0; // Max memory to use (leaving room for OS)
...
public:
  ...
  static void printSystemInfo(const SystemInfo &info) {
    ...
    std::cout << "  Cache Line Size: " << info.cacheLineSize << " bytes"
              << std::endl;

    std::cout << "\nMemory:" << std::endl;
    std::cout << "  Total RAM: " << formatBytes(info.totalPhysicalMemory)
              << std::endl;
    std::cout << "  Available RAM: "
              << formatBytes(info.availablePhysicalMemory) << std::endl;
    std::cout << "  Total Virtual: " << formatBytes(info.totalVirtualMemory)
              << std::endl;
    std::cout << "  Available Virtual: "
              << formatBytes(info.availableVirtualMemory) << std::endl;

    std::cout << "\nRecommended Limits:" << std::endl;
    std::cout << "  Heap Size: " << formatBytes(info.recommendedHeapSize)
              << std::endl;
    std::cout << "  Max Memory: " << formatBytes(info.recommendedMaxMemory)
              << std::endl;
    std::cout << "  Worker Threads: " << info.recommendedThreadCount
              << std::endl;
    std::cout << "=========================" << std::endl;
  }
...
private:
  ...
  static void detectMemoryInfo(SystemInfo &info) {
    ...
    if (sysinfo(&memInfo) == 0) {
      ...
      info.availableVirtualMemory =
          (memInfo.freeram + memInfo.freeswap) * memInfo.mem_unit;
    }
#endif
  }
  ...
  static void calculateRecommendedLimits(SystemInfo &info) {
    ...
    info.recommendedHeapSize =
        std::min(info.totalPhysicalMemory / 4,
                 static_cast<std::size_t>(2ULL * 1024 * 1024 * 1024));

    // Recommended max memory (75% of available RAM)
    info.recommendedMaxMemory =
        static_cast<std::size_t>(info.availablePhysicalMemory * 0.75);
    ...
    info.recommendedThreadCount =
        std::min(info.recommendedThreadCount, info.logicalCores);
  }
...
struct MemoryManagerAutoConfig : public MemoryManagerConfig {
  ...
  void configureFromSystem() {
    ...
    std::cout << "  Total RAM: "
              << (sysInfo.totalPhysicalMemory / (1024.0 * 1024.0 * 1024.0))
              << " GB" << std::endl;
    std::cout << "  Available RAM: "
              << (sysInfo.availablePhysicalMemory / (1024.0 * 1024.0 * 1024.0))
              << " GB" << std::endl;
    std::cout << "  Memory Budget: " << (totalBudget / (1024.0 * 1024.0))
              << " MB (" << (memoryUsagePercent * 100) << "% of available)"
              << std::endl;
    ...
    debugHeapSize =
        std::min(debugHeapSize, static_cast<MemorySize>(8 * 1024 * 1024));
    frameStackSize =
        std::min(frameStackSize, static_cast<MemorySize>(4 * 1024 * 1024));
    frameLinearSize =
        std::min(frameLinearSize, static_cast<MemorySize>(4 * 1024 * 1024));
    renderingPoolSize =
        std::min(renderingPoolSize, static_cast<MemorySize>(16 * 1024 * 1024));
    physicsPoolSize =
        std::min(physicsPoolSize, static_cast<MemorySize>(8 * 1024 * 1024));
#else
    ...
    std::cout << "  Configured heap: " << (mainHeapSize / (1024.0 * 1024.0))
              << " MB" << std::endl;
    std::cout << "  Frame buffers: " << static_cast<int>(frameBufferCount)
              << std::endl;
