...
namespace engine::resources {
    ...
    StreamableResource::StreamableResource(const ResourceID id, const std::string& name, const ResourceType type)
        : Resource(id, name, type)
          , streamingState_(StreamingState::NOT_STREAMING)
          , streamingPriority_(StreamingPriority::NORMAL)
          , currentLOD_(UINT32_MAX)
          , importance_(1.0f)
    ...
    bool StreamableResource::isLODLoaded(const std::uint32_t lodLevel) const {
        ...
        const std::uint32_t currentLODLevel = currentLOD_.load(std::memory_order_acquire);
        ...
        // LOD is loaded if current LOD is equal or better (lower number = higher detail)
    ...
    void StreamableResource::setStreamingPriority(const StreamingPriority priority) {
    ...
    void StreamableResource::setImportance(const float requiredImportance) {
        importance_.store(std::clamp(requiredImportance, 0.0f, 1.0f), std::memory_order_release);
    }

    std::chrono::steady_clock::time_point StreamableResource::getLastAccessTime() const {
        return lastAccessTime_.load(std::memory_order_acquire);
    }

    void StreamableResource::updateLastAccessTime() const {
        lastAccessTime_.store(std::chrono::steady_clock::now(), std::memory_order_release);
    }
    ...
    bool StreamableResource::shouldStreamOut(float memoryPressure) const {
        ...
        if (const auto timeSinceAccess = std::chrono::duration_cast<std::chrono::seconds>(now - lastAccess);
            timeSinceAccess.count() < 5 && memoryPressure < 0.9f) {
            return false;
        }

        // More likely to stream out with higher memory pressure and lower importance
        const float streamOutProbability = memoryPressure * (1.0f - resourceImportance);
    ...
    float StreamableResource::getStreamingCost(const std::uint32_t lodLevel) const {
        ...
        constexpr float streamingSpeed = 100.0f * 1024.0f * 1024.0f; // Bytes per second
    ...
    void StreamableResource::recordStreamingOperation(const bool isStreamIn, const std::chrono::microseconds duration) {
        if (isStreamIn) {
