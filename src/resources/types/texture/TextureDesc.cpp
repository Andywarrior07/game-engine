...
bool TextureSamplerState::operator==(const TextureSamplerState &other) const {
  return minFilter == other.minFilter && magFilter == other.magFilter &&
         ...
         anisotropy == other.anisotropy && lodBias == other.lodBias &&
         minLod == other.minLod && maxLod == other.maxLod &&
         std::memcmp(borderColor, other.borderColor, sizeof(borderColor)) ==
             0 &&
...
std::size_t TextureDesc::calculateMemorySize() const {
  if (width == 0 || height == 0 || depth == 0 || arraySize == 0 ||
      mipLevels == 0) {
    return 0;
  }
    ...
    for (std::uint32_t mipLevel = 0; mipLevel < mipLevels; ++mipLevel) {
      ...
      if (TextureFormatUtils::isCompressed(format)) {
        ...
        std::uint32_t bytesPerPixel =
            TextureFormatUtils::getBytesPerPixel(format);
  ...
  // Para cubemaps, multiplicar por 6 caras (si no está ya incluido en
  // arraySize)
...
bool TextureDesc::validate() const {
  ...
  std::uint32_t maxPossibleMips =
      TextureFormatUtils::calculateMipLevels(width, height);
  ...
  if (hasUsage(usage, TextureUsage::IMMUTABLE) &&
      (hasUsage(usage, TextureUsage::CPU_WRITE) ||
       hasUsage(usage, TextureUsage::SHADER_WRITE))) {
