...
namespace engine::resources {
    ...
    bool ResourceDependencyGraph::addDependency(const ResourceID resource, const ResourceID dependency) {
        // Check for self-dependency
        if (resource == dependency) {
            std::cerr << "Warning: Cannot add self-dependency for resource " << resource << std::endl;
            return false;
        }

        // Check if a would create cycle
        if (wouldCreateCycle(resource, dependency)) {
            std::cerr << "Warning: Adding dependency would create cycle: "
                << resource << " -> " << dependency << std::endl;
            return false;
        }
    ...
    bool ResourceDependencyGraph::addDependencies(const ResourceID resource, const std::vector<ResourceID>& deps) {
        bool allAdded = true;
    ...
    void ResourceDependencyGraph::removeDependency(const ResourceID resource, const ResourceID dependency) {
        std::unique_lock lock(mutex_);
        ...
        if (const auto depentIt = dependents_.find(dependency); depentIt != dependents_.end()) {
    ...
    void ResourceDependencyGraph::removeResource(const ResourceID resource) {
        ...
        if (const auto depIt = dependencies_.find(resource); depIt != dependencies_.end()) {
            for (ResourceID dep : depIt->second) {
                if (const auto depentIt = dependents_.find(dep); depentIt != dependents_.end()) {
                    depentIt->second.erase(resource);
                    if (depentIt->second.empty()) {
                        dependents_.erase(depentIt);
                    }
        ...
        if (const auto depentIt = dependents_.find(resource); depentIt != dependents_.end()) {
    ...
    std::vector<ResourceID> ResourceDependencyGraph::getDependencies(const ResourceID resource) const {
    ...
    std::vector<ResourceID> ResourceDependencyGraph::getDependents(const ResourceID resource) const {
        std::shared_lock lock(mutex_);
    ...
    std::vector<ResourceID> ResourceDependencyGraph::getAllDependencies(const ResourceID resource) const {
        std::shared_lock lock(mutex_);
    ...
    std::vector<ResourceID> ResourceDependencyGraph::getAllDependents(const ResourceID resource) const {
        std::shared_lock lock(mutex_);
    ...
    bool ResourceDependencyGraph::dependsOn(const ResourceID resource, const ResourceID dependency) const {
        auto allDeps = getAllDependencies(resource);
        return std::ranges::find(allDeps, dependency) != allDeps.end();
    }

    std::size_t ResourceDependencyGraph::getDependencyDepth(const ResourceID resource) const {
        std::shared_lock lock(mutex_);
    ...
    std::vector<ResourceID> ResourceDependencyGraph::getLoadingOrder(
        ...
        if (result.size() != resources.size()) {
            std::cerr << "Warning: Circular dependency detected! Only "
                << result.size() << " of " << resources.size()
                << " resources can be loaded." << std::endl;
        }
    ...
    bool ResourceDependencyGraph::wouldCreateCycle(const ResourceID resource, const ResourceID dependency) const {
        // Check if dependency already depends on resource (directly or indirectly)
        return dependsOn(dependency, resource);
    }

    std::vector<std::vector<ResourceID>> ResourceDependencyGraph::findAllCycles() const {
    ...
    std::vector<ResourceID> ResourceDependencyGraph::getRootResources() const {
        ...
        for (ResourceID res : allResources) {
            if (!dependencies_.contains(res) ||
                dependencies_.at(res).empty()) {
                roots.push_back(res);
            }
        }
    ...
    std::vector<ResourceID> ResourceDependencyGraph::getLeafResources() const {
        ...
        for (ResourceID res : allResources) {
            if (!dependents_.contains(res) ||
                dependents_.at(res).empty()) {
    ...
    bool ResourceDependencyGraph::validate() const {
        ...
        for (const auto& [resource, deps] : dependents_) {
            for (ResourceID dep : deps) {
                if (const auto it = dependencies_.find(dep); it == dependencies_.end() || !it->second.contains(resource)) {
    ...
    bool ResourceDependencyGraph::hasCycleDFS(ResourceID node,
                                              std::unordered_set<ResourceID>& visited,
    ...
    void ResourceDependencyGraph::getAllDependenciesDFS(ResourceID resource,
                                                        std::unordered_set<ResourceID>& visited,
    ...
    void ResourceDependencyGraph::getAllDependentsDFS(ResourceID resource,
                                                      std::unordered_set<ResourceID>& visited,
                                                      std::vector<ResourceID>& result) const {
        if (const auto it = dependents_.find(resource); it != dependents_.end()) {
            for (ResourceID dep : it->second) {
