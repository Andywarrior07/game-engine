...
public:
   ...
   */
  bool addDependencies(ResourceID resource,
                       const std::vector<ResourceID> &deps);
   ...
   * @param resource The resource to query
   * @return List of all resource IDs that this resource depends on (directly or
   * indirectly)
   */
   ...
   * @param resource The resource to query
   * @return List of all resource IDs that depend on this resource (directly or
   * indirectly)
   */
  ...
  getLoadingOrder(const std::vector<ResourceID> &resources) const;

  /**
   * @brief Get unloading order for a set of resources (reverse topological
   * sort)
   * @param resources List of resources to unload
   * @return Ordered list of resources (dependents first)
   */
  std::vector<ResourceID>
  getUnloadingOrder(const std::vector<ResourceID> &resources) const;
...
private:
  ...
  std::unordered_map<ResourceID, std::unordered_set<ResourceID>>
      dependencies_; // resource -> what it depends on
  std::unordered_map<ResourceID, std::unordered_set<ResourceID>>
      dependents_; // resource -> what depends on it

  // Thread safety
  mutable std::shared_mutex mutex_;

  // Helper methods
  bool hasCycleDFS(ResourceID node, std::unordered_set<ResourceID> &visited,
  ...
  std::size_t getDependencyDepthDFS(
      ResourceID resource,
