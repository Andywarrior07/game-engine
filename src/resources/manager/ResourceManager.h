...
public:
  ...
  template <typename T>
  ResourceHandle<T>
  loadById(ResourceID id, std::string path,
  ...
  // Get resource
  template <typename T> ResourcePtr<T> getResource(ResourceID id) {
  ...
  template <typename T> ResourcePtr<T> getResource(const std::string &name) {
...
private:
  ...
  // Caches
  std::unordered_map<ResourceType, std::unique_ptr<ResourceCache<Resource>>>
      caches_;
  ...
  // Pending loads
  std::unordered_map<ResourceID, std::future<ResourcePtr<Resource>>>
      pendingLoads_;
  ...
  IResourceLoader *selectLoader(const std::string &path,
                                ResourceType type) const;

  // Loading implementation
  ResourcePtr<Resource> loadSync(ResourceID id, std::string path,
                                 ResourceType type);
  std::future<ResourcePtr<Resource>> loadAsync(ResourceID id,
  ...
  // Memory allocation
  void *allocateResourceMemory(ResourceSize size,
                               memory::MemoryCategory category) const;
  void deallocateResourceMemory(void *ptr,
                                memory::MemoryCategory category) const;
};
} // namespace engine::resources
