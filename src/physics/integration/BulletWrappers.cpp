...
BulletShapeWrapper::~BulletShapeWrapper() { release(); }
...
BulletBodyWrapper::~BulletBodyWrapper() { release(); }
...
BulletConstraintWrapper::~BulletConstraintWrapper() { release(); }
...
std::unique_ptr<btCollisionShape, BulletFactory::ShapeDeleter>
BulletFactory::createShape(btCollisionShape *shape) {
  return std::unique_ptr<btCollisionShape, ShapeDeleter>(
      shape, [](const btCollisionShape *s) { delete s; });
}

std::unique_ptr<btRigidBody, BulletFactory::BodyDeleter>
BulletFactory::createBody(btRigidBody *body) {
  return std::unique_ptr<btRigidBody, BodyDeleter>(body, [](btRigidBody *b) {
    if (b->getMotionState()) {
...
std::unique_ptr<btTypedConstraint, BulletFactory::ConstraintDeleter>
BulletFactory::createConstraint(btTypedConstraint *constraint) {
