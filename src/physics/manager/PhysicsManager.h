...
public:
  ...
  // TODO: este metodo esta en PhysicsWorld, quiza deberia hacer lo mismo que
  // raycast, y llamar mainworld->createRigidBody
  /**
  ...
  // TODO: este metodo esta en PhysicsWorld, quiza deberia hacer lo mismo que
  // raycast, y llamar mainworld->destroyRigidBody
  /**
  ...
  // TODO: este metodo esta en PhysicsWorld, quiza deberia hacer lo mismo que
  // raycast, y llamar mainworld->findBodyById
  /**
  ...
  raycastAll(const Vec3 &from, const Vec3 &to,
             const QueryFilter &filter = QueryFilter()) const;
  ...
  overlapSphere(const Vec3 &center, Float radius,
                const QueryFilter &filter = QueryFilter()) const;
   ...
   */
  std::vector<RigidBody *>
  overlapBox(const Vec3 &center, const Vec3 &halfExtents,
   ...
   */
  bool sweepCapsule(const Vec3 &from, const Vec3 &to, Float radius,
                    Float height, SweepResult &result,
   ...
   */
  CharacterController *
  createCharacterController(const CharacterControllerParams &params);
...
private:
   ...
   */
  CollisionShape *
  createCollisionShapeInternal(const ShapeCreationParams &params);
