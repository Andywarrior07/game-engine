/**
 * @file Primitives.h
 * @brief Geometric primitive definitions for collision detection and spatial
 * queries
 *
...
struct Ray {
  ...
  Ray(const Vec3 &origin, const Vec3 &direction,
      const Float maxDistance = INFINITY_VALUE<Float>) noexcept
...
struct LineSegment {
  ...
  [[nodiscard]] Vec3 getDirection() const noexcept { return end - start; }
  ...
  [[nodiscard]] Vec3 getCenter() const noexcept { return (start + end) * 0.5f; }
};
...
struct AABB {
   ...
   */
  static AABB fromCenterExtents(const Vec3 &center,
  ...
  [[nodiscard]] Vec3 getCenter() const noexcept { return (min + max) * 0.5f; }

  [[nodiscard]] Vec3 getSize() const noexcept { return max - min; }
...
struct OBB {
  ...
  OBB(const Vec3 &center, const Vec3 &halfExtents,
      const Quat &orientation = Quat(1, 0, 0, 0)) noexcept
  ...
  explicit OBB(const AABB &aabb) noexcept
      : center(aabb.getCenter()), halfExtents(aabb.getHalfExtents()),
        orientation(1, 0, 0, 0) {}
  ...
  [[nodiscard]] std::array<Vec3, 3> getAxes() const noexcept {
    ...
    return {{Vec3(rotMatrix[0]), Vec3(rotMatrix[1]), Vec3(rotMatrix[2])}};
  }
  ...
  [[nodiscard]] std::array<Vec3, 8> getCorners() const noexcept {
    ...
    for (int i = 0; i < 8; ++i) {
      Vec3 localCorner((i & 1) ? halfExtents.x : -halfExtents.x,
...
struct Frustum {
   ...
   */
  explicit Frustum(const Mat4 &viewProj) noexcept { extractPlanes(viewProj); }
  ...
  void extractPlanes(const Mat4 &viewProj) noexcept {
    // Left plane
    planes[PLANE_LEFT] = Plane(Vec3(viewProj[0][3] + viewProj[0][0],
                                    viewProj[1][3] + viewProj[1][0],
                                    viewProj[2][3] + viewProj[2][0]),
                               viewProj[3][3] + viewProj[3][0]);

    // Right plane
    planes[PLANE_RIGHT] = Plane(Vec3(viewProj[0][3] - viewProj[0][0],
                                     viewProj[1][3] - viewProj[1][0],
                                     viewProj[2][3] - viewProj[2][0]),
                                viewProj[3][3] - viewProj[3][0]);

    // Top plane
    planes[PLANE_TOP] = Plane(Vec3(viewProj[0][3] - viewProj[0][1],
                                   viewProj[1][3] - viewProj[1][1],
                                   viewProj[2][3] - viewProj[2][1]),
                              viewProj[3][3] - viewProj[3][1]);

    // Bottom plane
    planes[PLANE_BOTTOM] = Plane(Vec3(viewProj[0][3] + viewProj[0][1],
                                      viewProj[1][3] + viewProj[1][1],
                                      viewProj[2][3] + viewProj[2][1]),
                                 viewProj[3][3] + viewProj[3][1]);

    // Near plane
    planes[PLANE_NEAR] = Plane(Vec3(viewProj[0][3] + viewProj[0][2],
                                    viewProj[1][3] + viewProj[1][2],
                                    viewProj[2][3] + viewProj[2][2]),
                               viewProj[3][3] + viewProj[3][2]);

    // Far plane
    planes[PLANE_FAR] = Plane(Vec3(viewProj[0][3] - viewProj[0][2],
                                   viewProj[1][3] - viewProj[1][2],
                                   viewProj[2][3] - viewProj[2][2]),
                              viewProj[3][3] - viewProj[3][2]);
