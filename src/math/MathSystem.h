...
namespace engine::math {
    ...
    public:
        explicit MathSystem(std::unique_ptr<noise::PerlinNoise> perlin,
                            ...
                            std::unique_ptr<TransformHierarchy> transformHierarchy): random_(std::make_unique<Random>()),
          perlinNoise_(std::move(perlin)),
          simplexNoise_(std::move(simplex)),
          worleyNoise_(std::move(worley)),
        ...
        [[nodiscard]] noise::PerlinNoise& getPerlinNoise() {
            return *perlinNoise_;
        }
        ...
        [[nodiscard]] noise::WorleyNoise& getWorleyNoise() {
            return *worleyNoise_;
        }
...
#ifdef _DEBUG
            if (!config_.validateMatrices) return true;

            for (int i = 0; i < 4; ++i) {
                for (int j = 0; j < 4; ++j) {
                    if (std::isnan(matrix[i][j])) return false;
                    if (std::isinf(matrix[i][j])) return false;
                }
            }
        ...
        [[nodiscard]] static Mat3 calculateNormalMatrix(const Mat4& modelMatrix) noexcept {
        ...
        [[nodiscard]] static Mat4 perspective(Float fov, Float aspect, Float nearPlane, Float farPlane) noexcept {
        ...
        [[nodiscard]] static Mat4 ortho(Float left, Float right, Float bottom, Float top, Float nearPlane,
                                        Float farPlane) noexcept {
            return buildOrthographicMatrix(left, right, bottom, top, nearPlane, farPlane);
        }
