...
PerlinNoise::PerlinNoise(const UInt seed) noexcept { initialize(seed); }
...
Float PerlinNoise::noise2D(Float x, Float y) const noexcept {
  ...
  const Float res =
      lerp(lerp(grad2D(p_[AA], x, y), grad2D(p_[BA], x - 1, y), u),
           lerp(grad2D(p_[AB], x, y - 1), grad2D(p_[BB], x - 1, y - 1), u), v);
...
Float PerlinNoise::fbm2D(const Float x, const Float y, const int octaves,
                         ...
                         const Float gain) const noexcept {
  Float value = 0;
  Float amplitude = 1;
  Float frequency = 1;
  Float maxValue = 0;
...
Float PerlinNoise::fbm3D(const Float x, const Float y, const Float z,
                         const int octaves, const Float lacunarity,
                         const Float gain) const noexcept {
  Float value = 0;
  Float amplitude = 1;
  Float frequency = 1;
...
Float PerlinNoise::grad2D(const int hash, const Float x,
...
Float PerlinNoise::grad3D(const int hash, const Float x, const Float y,
